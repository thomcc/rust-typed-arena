...
impl<T> Arena<T> {
        ...
        I: IntoIterator<Item = T>,
    {
        ...
        // Avoid indexing directly, which creates a reference to the
        // entire slice, invalidating the existing references.
        // The returned reference receives the lifetime of `self`,
        // which is OK because we’re careful to never move items
        ...
        unsafe {
            slice::from_raw_parts_mut(
                chunks.current.as_mut_ptr().add(next_item_index),
                chunks.current.len() - next_item_index,
            )
        }
    }
